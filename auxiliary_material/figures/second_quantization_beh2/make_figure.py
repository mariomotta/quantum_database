...
def custom_plot(ax,style,d,x,y):
    ax.plot(x,y,color=style[str(d)]['color'],ls=style[str(d)]['s'],marker=style[str(d)]['marker'],label=style[str(d)]['label'],ms=2) #style[str(d)]['ms'])
...
for c in [0,1,2,3]:
    ...
    fill_panel(ax[4,c],r'$R$ $[\AA]$',[0.5,4.5],[0.5,1.5,2.5,3.5,4.5],['0.5','1.5','2.5','3.5','4.5'],yl,[-2,2],[-2,-1,0,1,2],lab)
    # ---------------- 
