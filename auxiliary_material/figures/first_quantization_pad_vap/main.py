...
for j,mol in enumerate(['bh','hf','beh2','h2o']):
    ...
    for depth in [3,4,5]:
        ...
        for jj,R in enumerate(data_mol['geometries']):
            data_mol['E_cascade_%d'%depth][jj] = np.loadtxt('../../../first_quantization/pad/variation_after_projection/circuits/cascade_%d/%s_%s_results.txt'%(depth,mol,str(R)))[0,0]
            data_mol['P_cascade_%d'%depth][jj] = np.loadtxt('../../../first_quantization/pad/variation_after_projection/circuits/cascade_%d/%s_%s_results.txt'%(depth,mol,str(R)))[1,0]
    ...
    for d in d_list:
        ...
        Y = np.array(data_mol['P_cascade_%d'%d])
...
fname = 'first_quantization_pad_vap.eps'
