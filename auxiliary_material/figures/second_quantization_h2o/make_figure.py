...
for c in [0,1,2,3]:
    if(c>0): lab = ['']*5
    else:    lab = ['-75.80','-75.65','-75.50','-75.35','-75.20']
    if(c>0): yl = ''
    else:    yl = r'$E$ [$\mathrm{E_h}$]'
    fill_panel(ax[0,c],'',[0.5,3.5],[0.5,1.5,2.5,3.5],['','','',''],yl,[-75.8,-75.2],[-75.8,-75.65,-75.5,-75.35,-75.2],lab)
    # ---------------- 
    if(c>0): lab = ['']*5
    else:    lab = ['0.00','0.05','0.10','0.15','0.20']
    if(c>0): yl = ''
    else:    yl = r'$E$-$E_{FCI}$ [$\mathrm{E_h}$]'
    fill_panel(ax[1,c],'',[0.5,3.5],[0.5,1.5,2.5,3.5],['','','',''],yl,[0,0.2],[0,0.05,0.1,0.15,0.2],lab)
    # ---------------- 
    if(c>0): lab = ['']*5
    else:    lab = ['-0.04','-0.02','0.00','0.02','0.04']
    if(c>0): yl = ''
    else:    yl = r'$N$-$N_{FCI}$'
    fill_panel(ax[2,c],'',[0.5,3.5],[0.5,1.5,2.5,3.5],['','','',''],yl,[-0.04,0.04],[-0.04,-0.02,0.00,0.02,0.04],lab)
    # ---------------- 
    if(c>0): lab = ['']*5
    else:    lab = ['0.00','1.50','3.00','4.50','6.00']
    if(c>0): yl = ''
    else:    yl = r'$S^2$-$S^2_{FCI}$ [$\hbar$]'
    fill_panel(ax[3,c],'',[0.5,3.5],[0.5,1.5,2.5,3.5],['','','',''],yl,[0,6],[0.00,1.50,3.00,4.50,6.00],lab)
    # ---------------- 
    if(c>0): lab = ['']*5
    else:    lab = ['-0.04','-0.02','0.00','0.02','0.04']
    if(c>0): yl = ''
    ...
    fill_panel(ax[4,c],r'$R$ $[\mathrm{\AA}]$',[0.5,3.5],[0.5,1.5,2.5,3.5],['0.5','1.5','2.5','3.5'],yl,[-0.04,0.04],[-0.04,-0.02,0.00,0.02,0.04],lab)
    # ---------------- 
    ax[0,0].text(0.75,0.25,'$R_y$ linear',horizontalalignment='center',verticalalignment='center',transform=ax[0,0].transAxes,fontsize=14)
    ax[0,1].text(0.75,0.25,  '$R_y$ full',horizontalalignment='center',verticalalignment='center',transform=ax[0,1].transAxes,fontsize=14)
