...
for depth in [1]:
  for dist in dist_list:
    ...
    p0 = np.loadtxt('/Users/mario/Documents/GitHub/QITE/qite_es/scf_calculations/qUCCSD/bh/R_%s/output_parameters.txt' % str(dist))
    ...
    print_results('unrestricted/singles_doubles/trotter/bh_reps_%d_%s_results.txt' % (depth,str(dist)),res_vqe,res_ee,dN=2)

    circuit = algo.get_optimal_circuit().decompose()
    print_circuit('unrestricted/singles_doubles/trotter/bh_reps_%d_%s_circuit.txt' % (depth,str(dist)),circuit)
