for flavor in 'restricted_closed_shell' 'unrestricted';
do
  for reverse in 'doubles_singles' 'singles_doubles';
  do
    for approximation in 'trotter' 'suzuki';
    do
      for R in 0.7 0.9 1.1 1.3 1.5 1.7 1.9 2.1 2.3 2.5 2.7 2.9 3.1 3.3;
      do
        for depth in 2;
        do
          cat > main.py <<***
...
if('${flavor}'=='restricted_closed_shell'):
...
var_form  = UCCSD(num_orbitals=core._molecule_info['num_orbitals'],num_particles=core._molecule_info['num_particles'],
                  ...
                  reverse_excitations=rev,expansion_mode='${approximation}',reps=${depth})
optimizer = COBYLA(maxiter=0)
depth = ${depth}
if(depth==1): p0 = np.loadtxt('/Users/mario/Documents/GitHub/QITE/qite_es/scf_calculations/qUCCSD/quccsd_flavors_h2o/${flavor}/${approximation}/ds_%s/R_%s/output_parameters.txt' % (str(rev),str(dist)))
else:         p0 = np.loadtxt('/Users/mario/Documents/GitHub/QITE/qite_es/scf_calculations/qUCCSD/quccsd_flavors_h2o_reps_2/${flavor}/${approximation}/ds_%s/R_%s/output_parameters.txt' % (str(rev),str(dist)))
...
res_vqe,res_ee = get_results(H_op,A_op,molecule,core,algo_result,outfile)
depth = ${depth}
print_results('${flavor}/${reverse}/${approximation}/h2o_reps_%d_%s_results.txt' % (depth,str(dist)),res_vqe,res_ee,dN=4)
circuit = algo.get_optimal_circuit().decompose()
print_circuit('${flavor}/${reverse}/${approximation}/h2o_reps_%d_%s_circuit.txt' % (depth,str(dist)),circuit)
***
          python main.py
        done
      done
