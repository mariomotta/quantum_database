...
def print_circuit(fname,circuit):
    ...
    for gate in circuit:
        ...
        if(gate_name!='barrier'):
           outf.write(str(gate_name)+' | '+' '.join([str(x) for x in qubits])+' | '+' '.join([str(x) for x in gate[0].params])+'\n')
    outf.close()
...
for depth in depth_list:
  for dist in dist_list:
    ...
    driver = PySCFDriver(atom="F 0.0000 0.0000 0.0000; H 0.0000 0.0000 {0}".format(dist),
                         unit=UnitsType.ANGSTROM,charge=0,spin=0,basis='sto-6g',
                         hf_method=HFMethodType.RHF,symgroup='coov',
                         outfile=outfile,
                         irrep_nelec={'A1':6,'E1x':2,'E1y':2},rho=rho)
    molecule  = driver.run()
    ...
    par = np.loadtxt('/Users/mario/Documents/GitHub/QITE/qite_es/scf_calculations/scf_calculations/Cascade_DEPTH_%d/hf/R_%s/output_parameters.txt' % (depth,str(dist)))
    var_form  = CustomVarForm(num_qubits=H_op.num_qubits,reps=depth,entanglement='linear',initial_state=init_state.construct_circuit())
    ...
    print_results('cascade_%d/hf_%s_results.txt' % (depth,str(dist)),res_vqe,res_ee,dN=2)

    circuit = algo.get_optimal_circuit()
    print_circuit('cascade_%d/hf_%s_circuit.txt' % (depth,str(dist)),circuit)
    print(p1)
    print(res_vqe,res_ee)
    print(dist,depth)
