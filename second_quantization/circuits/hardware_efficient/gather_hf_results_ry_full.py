...
for depth in depth_list:
  for dist in dist_list:
    ...
    par = np.loadtxt('/Users/mario/Documents/GitHub/QITE/qite_es/scf_calculations/scf_calculations/Ry_full_DEPTH_%d/hf/R_%s/output_parameters.txt' % (depth,str(dist)))
    ...
    print_results('ry_full_%d/hf_%s_results.txt' % (depth,str(dist)),res_vqe,res_ee,dN=2)

    circuit = var_form.bind_parameters(p1)
    print_circuit('ry_full_%d/hf_%s_circuit.txt' % (depth,str(dist)),circuit)
