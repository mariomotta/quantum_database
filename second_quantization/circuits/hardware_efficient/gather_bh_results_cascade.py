...
for depth in depth_list:
  for dist in dist_list:
    ...
    driver    = PySCFDriver(atom="B 0.0000 0.0000 0.0000; H 0.0000 0.0000 {0}".format(dist),
                            ...
                            outfile=outfile,irrep_nelec={'A1':6},rho=rho,sort_by_irrep=np.abs(dist-2.3)<1e-4 and depth==1)
    molecule  = driver.run()
    ...
    par = np.loadtxt('/Users/mario/Documents/GitHub/QITE/qite_es/scf_calculations/Cascade_DEPTH_%d/bh/R_%s/output_parameters.txt' % (depth,str(dist)))
    ...
    print_results('cascade_%d/bh_%s_results.txt' % (depth,str(dist)),res_vqe,res_ee,dN=2)

    circuit = algo.get_optimal_circuit()
    print_circuit('cascade_%d/bh_%s_circuit.txt' % (depth,str(dist)),circuit)
    print(p1)
