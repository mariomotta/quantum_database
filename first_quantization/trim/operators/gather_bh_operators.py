...
for dist in dist_list:
    ...
    mol = gto.Mole()
    mol.build(atom     = [['B',(0.0000,0.0000,0.0000)],
    ...
    H_fci,S_fci,nqubit,w = trim(H_fci,S_fci,return_w=True)
    d4 = H_fci.shape[0]
    e4 = LA.eigh(H_fci)[0][0]

    H_op = matrix_to_qubit_operator(H_fci)
    write_to_file(H_op,0,'bh_%s_h.txt'%dist)
    info.append([d1,d2,d3,d4,e1,e4,w])

outf = open('bh_info.txt','w')
for d1,d2,d3,d4,e1,e4,w in info:
    outf.write('%d %d %d %d %.12f %.12f %.12f \n' % (d1,d2,d3,d4,e1,e4,w))
outf.close()
